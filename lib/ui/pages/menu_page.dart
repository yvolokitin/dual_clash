...
Future<void> showAnimatedMainMenuDialog({required BuildContext context, required GameController controller}) async {
                        ...
                        child: Column(
                          ...
                          children: [
                            Row(
                              children: [
                                const Spacer(),
                                const Text('Game Menu', style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w800)),
                                const Spacer(),
                                Container(
                                  width: 36,
                                  height: 36,
                            ...
                            _StatsAnimatedBox(totalScore: controller.totalUserScore, totalPlayTimeMs: controller.totalPlayTimeMs),
                            ...
                            _MenuTile(
                              ...
                              onTap: () {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Game Challenge is coming soon')));
                              },
                            ),
                            _MenuTile(
                              ...
                              label: 'Load game',
                              onTap: () async {
                                await showLoadGameDialog(context: context, controller: controller);
                              },
                            ),
                            _MenuTile(
                              icon: Icons.history,
...
class _StatsAnimatedBox extends StatelessWidget {
  ...
  const _StatsAnimatedBox({required this.totalScore, required this.totalPlayTimeMs});
        ...
        return AnimatedContainer(
          ...
          child: Center(
            child: Wrap(
              ...
              children: [
                chip(
                  icon: Image.asset('assets/icons/points-removebg.png', width: iconSize, height: iconSize),
                  text: '$totalScore',
                ),
                chip(
                  icon: Image.asset('assets/icons/duration-removebg.png', width: iconSize, height: iconSize),
...
Future<bool?> showLoadGameDialog({required BuildContext context, required GameController controller}) async {
  final bg = AppColors.bg;
    ...
    transitionBuilder: (ctx, anim, secondaryAnim, child) {
      ...
      return FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snap) {
          ...
          List<Map<String, dynamic>> initialItems = snap.data ?? const <Map<String, dynamic>>[];
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    Row(
                                      children: [
                                        const Spacer(),
                                        const Text('Load game', style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w800)),
                                        const Spacer(),
                                        Container(
                                          width: 36,
                                          height: 36,
                                    ...
                                    Expanded(
                                      child: localItems.isEmpty
                                          ? const Center(child: Text('No saved games', style: TextStyle(color: Colors.white70)))
                                          : ListView.separated(
                                              itemCount: localItems.length,
                                              separatorBuilder: (_, __) => const SizedBox(height: 10),
                                              itemBuilder: (context, index) {
                                                final it = localItems[index];
                                                final when = DateTime.fromMillisecondsSinceEpoch(it['ts'] as int);
                                                final title = it['name'] as String? ?? 'Saved game';
                                                final subtitle = '${when.toLocal()} • Turn: ${(it['current'] as String)}';
                                                  ...
                                                  child: ListTile(
                                                    ...
                                                    trailing: Row(
                                                      ...
                                                      children: [
                                                        ...
                                                        IconButton(
                                                          tooltip: 'Play',
                                                          icon: Image.asset('assets/icons/play-removebg.png', width: 31, height: 31),
                                                          onPressed: () async {
                                                            await controller.loadSavedGameById(id);
                                                            if (context.mounted) {
                                                        ...
                                                        if (deletingId == id)
                                                          const SizedBox(width: 31, height: 31, child: CircularProgressIndicator(strokeWidth: 2))
                                                        else
                                                          IconButton(
                                                            tooltip: 'Delete',
                                                            icon: Image.asset('assets/icons/delete-removebg.png', width: 31, height: 31),
                                                            onPressed: () async {
                                                              ...
                                                              if (confirm == true) {
                                                                ...
                                                                final ok = await controller.deleteSavedGameById(id);
                                                                if (ok) {
                                                                  setState(() {
